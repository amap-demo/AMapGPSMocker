...
@interface AGMManualMode()<AGMDriveCarEmulatorViewControllerDelegate>
{
...
@property (nonatomic,strong) AGMDriveCarEmulatorViewController *driverCarVC;

@end

@implementation AGMManualMode
...
- (CLLocationCoordinate2D)gengerateNextPointWithCurrentPosition:(CLLocationCoordinate2D)currentPosition
    ...
    //角度标准化为0-359.9之间
    direction = [AGMCaclUtil normalizeDegree:direction];
    //角度变换成弧度
...
- (CLLocationCoordinate2D)findCoordinateFromIndex:(unsigned long)startIndex
    ...
    for (; i < _count - 1; i++) {
        ...
        if (totalDistance <= dis) {
            ...
            resultCoordiante = [AGMCaclUtil coordFromCoord:*(_oriCoordinates + i)
                                                 toCoord:*(_oriCoordinates + i + 1)
                                                withRate:totalDistance / dis];
            break;
        } else {
            totalDistance -= dis;
        }
